        ...
        return (
          ...
          >
            <button
              ...
              style={{ background: isOpen ? 'var(--color-blush)' : 'white' }}
            >
              <div className="flex items-center gap-3 min-w-0">
                <span
                  className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold"
                  style={{ background: isOpen ? 'var(--color-coral)' : 'var(--color-peach)', color: isOpen ? 'white' : 'var(--color-navy)' }}
                >
                  {day.day}
                </span>
                <span className="font-semibold text-sm truncate" style={{ fontFamily: 'var(--font-display)', color: 'var(--color-navy)' }}>
                  {day.title}
                </span>
              </div>
              <svg
            ...
            {isOpen && (
              <div className="px-5 pb-5 pt-1" style={{ background: 'white' }}>
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {day.stay && (
                    <span className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full" style={{ background: 'var(--color-gray-soft)', color: 'var(--color-navy)' }}>
                ...
                {day.note && (
                  <div
                    ...
                    style={{ borderLeft: '3px solid var(--color-coral)', background: 'var(--color-blush)', color: 'var(--color-navy)' }}
                  >
                    {day.note}
                  </div>
                )}
