...
export const GET: APIRoute = async ({ url }) => {
  ...
  const row = getDb()
    .prepare('SELECT id, status, tier_id, batch_id, trip_name, source, amount_paid, payment_status, amount_refunded, total_amount, sharing_option FROM registrations WHERE email = ? ORDER BY id DESC LIMIT 1')
